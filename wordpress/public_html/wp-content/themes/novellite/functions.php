...
add_action('init', 'NovelLite_add_stylesheet');
/* ----------------------------------------------------------------------------------- */
/* jQuery Enqueue */
/* ----------------------------------------------------------------------------------- */
 ...
 */
function NovelLite_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( __( 'Page %s', 'novellite' ), max( $paged, $page ) );
	}

	return $title;
}
add_filter( 'wp_title', 'NovelLite_wp_title', 10, 2 );
